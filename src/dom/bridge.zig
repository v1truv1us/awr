...
const BRIDGE_POLYFILL =
    ...
    \\    whitesmoke:[245,245,245],yellow:[255,255,0],yellowgreen:[154,205,50],transparent:[0,0,0,0]
    \\  };
    \\  function __awr_hsl_to_rgb__(h, s, l) {
    \\    h = ((h % 360) + 360) % 360;
    ...
    \\    } else if (/^hsla?\(/.test(v)) {
    \\      const mh = /^hsla?\(([^)]+)\)$/.exec(v);
    \\      if (mh) {
    \\        const p = mh[1].split(',').map(s => s.trim());
    \\        if (p.length >= 3) {
    \\          rgb = __awr_hsl_to_rgb__(parseFloat(p[0]), parseFloat(p[1])/100, parseFloat(p[2])/100);
    \\          rgb.push(p[3] !== undefined ? parseFloat(p[3]) : undefined);
    \\        }
    \\      }
    \\    } else {
    \\      const m = /^rgba?\(([^)]+)\)$/.exec(v);
    ...
    \\    if (typeof value === 'string' && value.toLowerCase() === 'currentcolor') {
    \\      // currentColor resolves to the element's computed `color`; on the
    \\      // color property itself it inherits.
    \\      if (css === 'color') {
    \\        return (el && el.parentElement)
    \\          ? getComputedStyle(el.parentElement).getPropertyValue('color') : __awr_initial__('color');
    \\      }
    \\      return el ? getComputedStyle(el).getPropertyValue('color') : 'rgb(0, 0, 0)';
    \\    }
    \\    if (value === 'inherit') {
    \\      return (el && el.parentElement)
